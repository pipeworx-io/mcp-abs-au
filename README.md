...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/abs_au_list_dataflows \
  -H 'Content-Type: application/json' \
  -d '{"search":"consumer price"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/abs_au_list_dataflows`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
