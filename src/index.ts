 ...
 *
 * Host: `api.data.abs.gov.au` does not resolve and has not for as long as we
 * have metrics — every call this pack made came back 530, which is what a
 * Worker fetch to an NXDOMAIN name looks like and is indistinguishable from
 * "the origin is down". The live host reverses the first two labels and adds a
 * /rest prefix: `data.api.abs.gov.au/rest`.
 *
 * SDMX-JSON quirk: this endpoint speaks SDMX-JSON **1.0**, not 2.0.0 — asking
 * for 2.0.0 gets a 406 that lists what it will actually serve. The two shapes
 * differ where it matters: 1.0 puts the structure at `data.structure`
 * (singular), 2.0.0 at `data.structures[0]`. We read either, because the ABS
 * has said it intends to move to 2.0.0.
 */
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Australian Bureau of Statistics');
}

const BASE = 'https://data.api.abs.gov.au/rest';
const UA = 'pipeworx-mcp-abs-au/1.0 (+https://pipeworx.io)';
const ACCEPT_STRUCTURE = 'application/vnd.sdmx.structure+json; charset=utf-8; version=1.0';
const ACCEPT_DATA = 'application/vnd.sdmx.data+json; charset=utf-8; version=1.0';
...
async function dataflowStructure(dataflowId: string, maxCodes: number): Promise<unknown> {
  ...
  // Dimensions carry a `conceptIdentity` urn rather than an inline concept, so
  // the human-readable name ("Measure", "Region") only exists in the concept
  // schemes the same response ships. Without this, every dimension name was
  // just its own id echoed back.
  const concepts = new Map<string, string>();
  for (const cs of data.conceptSchemes ?? []) {
    for (const c of cs.concepts ?? []) {
      if (c.id) concepts.set(c.id, c.name ?? c.names?.en ?? c.id);
    }
  }
  ...
  return {
    ...
    dimensions: dims.map((d) => {
      ...
      return {
        id: d.id,
        name: d.concept?.name ?? concepts.get(d.id) ?? d.id,
...
async function getData(
  ...
  // for this key/period" plus a raw dump — a wrong answer wearing the shape of
  // a right one, which is worse than the 530 that was hiding it.
  const structure = json?.data?.structure ?? (json?.data?.structures ?? [])[0];
...
async function absGet(path: string, accept: string): Promise<unknown> {
  ...
  if (!res.ok) {
    ...
    // A 406 here means ABS changed which SDMX-JSON version it serves; its body
